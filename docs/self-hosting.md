...
- `communityChannel`: optional external-community integration; set `enabled: false` to remove it, or configure the provider name, browser-visible URL, CTA labels, rules, eligibility, consent wording, admin authority, and minors guidance
...
The community-channel rules gate is an acknowledgement step, not an authentication boundary: its destination URL is delivered to the browser. Account invitation URLs are different; Supabase generates them after members sign in, and they do not live in public configuration or tracked source files. The legal pages are starter templates, not legal advice; review and replace them for your operator, providers, users, and jurisdiction before launch.
...
In `supabase/config.toml`, add your own exact production `/auth/confirm` URL to `additional_redirect_urls`. The tracked template contains only local callback URLs.
