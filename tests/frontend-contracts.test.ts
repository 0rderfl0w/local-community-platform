...
describe('launch frontend contracts', () => {
  ...
  test('the header identity and generic favicon stay configurable', async () => {
    ...
    expect(favicon).toContain('<title id="title">Local Community Platform</title>');
    expect(favicon).not.toContain('Braga AI Builders');
  ...
  test('landing and members routes expose only the requested community CTAs', async () => {
    ...
    expect(home).toContain('communityChannel.joinLabel');
    expect(home.match(/data-community-join/g)).toHaveLength(2);
    expect(home).not.toContain('communityConfig.communityChannel.url');
    expect(config).toContain('https://chat.whatsapp.com/YOUR_INVITE_CODE');
    ...
    expect(members).toContain('communityConfig.communityChannel.joinLabel');
    expect(members).toContain('data-community-join');
    expect(members).not.toContain('communityConfig.communityChannel.url');
  ...
  test('email-link sign in and signup stay short, explicit, and passwordless', async () => {
    ...
    expect(tabs).toContain('data-community-join');
    expect(tabs).not.toContain('href={communityConfig.communityChannel.url}');
    ...
    expect(form).toContain('New here?');
    expect(form).toContain('Sign Up →');
    expect(form).not.toContain('data-community-join');
    expect(form).not.toContain('href={communityConfig.communityChannel.url}');
